...
push (uint8_t *kpage, size_t *ofs, const void *buf, size_t size)
{
  ...
  *ofs -= padsize;
  uint8_t* kpage_ptr = kpage + *ofs;
  if (buf == NULL)
      kpage_ptr = NULL;
  else
      memcpy(kpage_ptr, buf, size);
  return (void*) (kpage_ptr); // replace this with the right return address
}
               ...
               void **esp)
{
  ...
  for (argc = 0, arg = strtok_r (cmd_line, " ", &save_ptr);
            arg != NULL; arg = strtok_r (NULL, " ", &save_ptr), ++argc){
    argv[argc] = push(upage, &ofs, arg, strlen(arg) + 1);
  }
  ...
  for (i = 0; i < argc; ++i){
    push(upage, &ofs, &argv[i], sizeof(char*));
  }
  argc--;
  char *tmp_ptr = upage + ofs;
  push(upage, &ofs, &(tmp_ptr), sizeof(char**));
  push(upage, &ofs, &argc, sizeof(int));
  push(upage, &ofs, 0, sizeof(void(*)()));
  free(argv);
